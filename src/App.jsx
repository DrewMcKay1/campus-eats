              ...
              <div style={{textAlign:"right",flexShrink:0,display:"flex",alignItems:"center",gap:10}}>
                ...
                <button onClick={()=>onRemoveAddedIngredient(i)} title="Remove this ingredient"
                  style={{border:"1px solid #FCA5A5",background:"#FFF1F1",color:"#DC2626",borderRadius:6,
                    padding:"5px 9px",fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"'Nunito Sans',sans-serif"}}>
                  🗑 Remove
                </button>
              </div>
            </div>
...
function ShoppingList({meals,owned,onClearList,checked,setChecked,hideOwned,setHideOwned,
  ...
  return (
    <div className="fade-up">
      ...
      {CAT_ORDER.filter(c=>groups[c]?.some(i=>!removedMap[i.item.toLowerCase()]&&(!hideOwned||!owned[i.item.toLowerCase()]))).map(cat=>{
                  ...
                  <div key={i}
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:8,marginTop:7}}>
                      ...
                      <button onClick={()=>onRemoveItem(k)} title="Remove from shopping list — I don't need this"
                        style={{border:"1px solid #FCA5A5",background:"#FFF1F1",color:"#DC2626",borderRadius:6,
                          padding:"4px 9px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Nunito Sans',sans-serif",
                          display:"flex",alignItems:"center",gap:4}}>🗑 Remove</button>
                    </div>
                  </div>
                );
              })}
      ...
      <p style={{fontSize:12.5,color:"#94A3B8",lineHeight:1.65,marginTop:6}}>
        💡 Tap <strong>?</strong> on any item to see which meals use it. Use <strong>+ / −</strong> to note how many you need to buy, and <strong>Remove</strong> if you don't need something at all.
      </p>
...
export default function App({ session }) {
  ...
  const stateSnapshot = useMemo(()=>({
    ...
    shopRemoved, shopQty, ownedQty, ingOverrides
  }),[allPlans,prefs,owned,eaten,favorites,dislikes,
      addedToList,customMeals,shopChecked,shopHideOwned,weekOff,selDay,
      shopRemoved,shopQty,ownedQty,ingOverrides]);
