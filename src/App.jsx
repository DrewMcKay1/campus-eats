...
/* ─── RECIPE MODAL ─── */
function AddIngRow({onAdd,onCancel}) {
    ...
    <div style={{background:"#F0FDF4",border:"1.5px solid #86EFAC",borderRadius:8,padding:"10px 12px",marginTop:8}}>
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:7,marginBottom:7}}>
        <div>
          ...
          <input value={item} onChange={e=>{setItem(e.target.value);setErr(false);}} placeholder="e.g. Lime"
...
function RecipeModal({meal,type,owned,overrides,onToggleHide,onAddIngredient,onRemoveAddedIngredient,onClose}) {
  ...
  return (
    <Modal onClose={onClose} width={580}>
      ...
      <div style={{overflow:"auto",flex:1,padding:"16px 22px 24px"}}>
        ...
        {showAddForm && <AddIngRow onAdd={ing=>{onAddIngredient(ing);setShowAddForm(false);}} onCancel={()=>setShowAddForm(false)}/>}
        {baseIngs.map((ing,i)=>{
...
/* ─── PANTRY VIEW ─── */
function PantryView({owned,onToggle,onSetAll,ownedQty,onQtyChange}) {
  const [search,setSearch] = useState("");
...
export default function App({ session }) {
  ...
  const viewContent = (
    <>
      ...
      {view==="pantry" && <PantryView owned={owned} onToggle={toggleOwned} onSetAll={setAllOwned} ownedQty={ownedQty} onQtyChange={changeOwnedQty}/>}
  ...
  const sharedModals = (
    <>
      {recModal && <RecipeModal meal={recModal.meal} type={recModal.type} owned={owned}
        overrides={ingOverrides[recModal.meal.id]}
